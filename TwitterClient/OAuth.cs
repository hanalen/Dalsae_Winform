...
namespace TwitterClient
{
	...
	class OAuth
	{
		...
		{
			if (instence == null) { instence = new TwitterClient.OAuth(); }
		...
		public OAuth()
		{
			...
			this.OAuthConsumerKey = DataManager.ConsumerKey;
			this.OAuthConsumerSecret = DataManager.ConsumerSecret;
		...
		public string GetHeader(BaseParameter parameter)
		{
			lock(lockObject)
			{
				Refresh();
		...
		private string CalcSignature(BaseParameter parameter)
		{
			...
			HMACSHA1 sha = new HMACSHA1(Encoding.ASCII.GetBytes(signKey));
			...
			string ret = Convert.ToBase64String(hashvalue);
			sha.Dispose();
			return ret;
		}
		...
		{
			return string.Format("{0}&{1}", DataManager.ConsumerSecret,
